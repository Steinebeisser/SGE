...
SGE_RESULT sge_vulkan_uniform_buffer_create(sge_render *render) {
        ...
        VkResult ubo_buffer_create_result = vkCreateBuffer(vk_context->device, &ubo_buffer_create_info, vk_context->sge_allocator, &vk_context->uniform_buffer);
        ...
        vkGetBufferMemoryRequirements(vk_context->device, vk_context->uniform_buffer, &memory_requirements);
        ...
        VkResult alloc_result = vkAllocateMemory(vk_context->device, &allocate_info, vk_context->sge_allocator, &vk_context->uniform_buffer_memory);
        if (alloc_result != VK_SUCCESS) {
                log_event(LOG_LEVEL_FATAL, "failed allocating memory to uniform buffer");
                return SGE_ERROR;
        }

        vkBindBufferMemory(vk_context->device, vk_context->uniform_buffer, vk_context->uniform_buffer_memory, 0);
