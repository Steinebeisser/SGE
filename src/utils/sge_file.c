...
SGE_BOOL create_directory_if_not_exists(char *dir_path) {
        ...
        if (stat(dir_path, &st) == -1) {
                ...
                log_internal_event(LOG_LEVEL_INFO, "Created dir: \"%s\", in path: %s", folder_name, dir_path);
        }
...
uint32_t *read_file_as_binary(const char *filepath, size_t *code_size) {
        ...
        if (file == NULL) {
                log_internal_event(LOG_LEVEL_FATAL, "Failed to open file with path: %s", filepath);
        }
        ...
        *code_size = ftell(file);
        ...
        if (bytes_read != *code_size) {
                log_internal_event(LOG_LEVEL_FATAL, "File size does not match read bytes");
        }
